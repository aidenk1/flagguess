...
<body>
    ...
    <div class="homepage">
        ...
        <a href="/guesstheflag/flaglist" class="button">List of Flags</a>
    </div>
    ...
    <div id="ending-screen">
        <div style="background-color: rgba(255, 255, 255, 0.8); padding: 20px; border-radius: 10px;">
            ...
            <button id="leaderboard-button" onclick="redirectToLeaderboard()">Leaderboard</button>
    ...
    <script>
        ...
        function showEndingScreen() {
            ...
            const userId = localStorage.getItem('userId'); // Assuming this retrieves the user ID
            const userName = userId ? `User ${userId}` : 'Anonymous'; // Set a default name if user ID is not available
            document.getElementById('submit-score-button').addEventListener('click', () => {
                submitScore(userName, score);
            });
        }
        function redirectToLeaderboard() {
            window.location.href = "/guesstheflag/leaderboard"; // Replace "https://example.com/leaderboard" with the actual URL of your leaderboard page
        }
async function submitScore(userName, userScore) {
    const apiUrl = "http://127.0.0.1:8086/api/score/";
    // Extract the user ID from userName if it includes a prefix
    const userId = extractUserId(userName);
    const data = {
        name: userId, // Send the extracted user ID
        score: userScore
    };
    try {
...
function extractUserId(userName) {
    // Check if userName starts with "User " and extract the user ID
    if (userName.startsWith('User ')) {
        return userName.substring(5); // Extract the user ID after removing "User "
    } else {
        return userName; // Return the userName as is if it doesn't start with "User "
    }
}
        function resetQuiz() {
